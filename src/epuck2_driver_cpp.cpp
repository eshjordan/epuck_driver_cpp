...
#define WHEEL_SEPARATION 0.053  // Separation between wheels, axis length (m).
...
private:
    ...
    rclcpp::Subscription<std_msgs::msg::UInt8MultiArray>::SharedPtr cmdRgbLedsSubscriber;

    std::shared_ptr<tf2_ros::TransformBroadcaster> br;
    std::shared_ptr<tf2_ros::StaticTransformBroadcaster> static_br;

    rclcpp::TimerBase::SharedPtr tf_timer;
    rclcpp::Time odomLastPublished = rclcpp::Time(0);
...
public:
    EpuckDriver() : rclcpp::Node("epuck_driver_cpp")
    {
        this->declare_parameter<int>("epuck2_id");
        this->declare_parameter<std::string>("epuck2_address");
        this->declare_parameter<uint16_t>("epuck2_port");
        this->declare_parameter<std::string>("epuck2_name");
        this->declare_parameter<double>("robot/xpos", 0.0);
        this->declare_parameter<double>("robot/ypos", 0.0);
        this->declare_parameter<double>("robot/theta", 0.0);
        this->declare_parameter<bool>("robot/camera", false);
        this->declare_parameter<bool>("robot/floor", false);

        br        = std::make_shared<tf2_ros::TransformBroadcaster>(this);
        static_br = std::make_shared<tf2_ros::StaticTransformBroadcaster>(this);
    }

    ~EpuckDriver() override { closeConnection(); }
    ...
    int initConnectionWithRobot()
    {
        ...
        robot_addr.sin_port        = htons(epuckPort);

        if (DEBUG_CONNECTION_INIT)
            fprintf(stderr, "Try to connect to %s:%d (TCP)\n", inet_ntoa(robot_addr.sin_addr),
                    htons(robot_addr.sin_port));
        ...
        // Set to non-blocking mode during connection otherwise it will block for too much time if the robot isn't ready
        ...
        while (expected_recv_packets > 0)
        {
            ...
            case 0x01: // Camera.
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "camera read correctly" << std::endl;
            ...
            case 0x02: // Sensors.
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "acc: " << accData[0] << "," << accData[1] << ","
                              << accData[2] << std::endl;

                // Compute acceleration
                mantis = (sensor[6] & 0xff) + ((sensor[7] & 0xffl) << 8) + (((sensor[8] & 0x7fl) | 0x80) << 16);
                exp    = (sensor[9] & 0x7f) * 2 + ((sensor[8] & 0x80) ? 1 : 0);
                if (sensor[9] & 0x80) { mantis = -mantis; }
                flt          = (mantis || exp) ? ((float)ldexp(mantis, (exp - 127 - 23))) : 0;
                acceleration = flt;
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "acceleration: " << acceleration << std::endl;

                // Compute orientation.
                mantis = (sensor[10] & 0xff) + ((sensor[11] & 0xffl) << 8) + (((sensor[12] & 0x7fl) | 0x80) << 16);
                exp    = (sensor[13] & 0x7f) * 2 + ((sensor[12] & 0x80) ? 1 : 0);
                if (sensor[13] & 0x80) mantis = -mantis;
                flt         = (mantis || exp) ? ((float)ldexp(mantis, (exp - 127 - 23))) : 0;
                orientation = flt;
                if (orientation < 0.0) orientation = 0.0;
                if (orientation > 360.0) orientation = 360.0;
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "orientation: " << orientation << std::endl;

                // Compute inclination.
                mantis = (sensor[14] & 0xff) + ((sensor[15] & 0xffl) << 8) + (((sensor[16] & 0x7fl) | 0x80) << 16);
                exp    = (sensor[17] & 0x7f) * 2 + ((sensor[16] & 0x80) ? 1 : 0);
                if (sensor[17] & 0x80) mantis = -mantis;
                flt         = (mantis || exp) ? ((float)ldexp(mantis, (exp - 127 - 23))) : 0;
                inclination = flt;
                if (inclination < 0.0) inclination = 0.0;
                if (inclination > 180.0) inclination = 180.0;
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "inclination: " << inclination << std::endl;
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "gyro: " << gyroRaw[0] << "," << gyroRaw[1] << ","
                              << gyroRaw[2] << std::endl;
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "mag: " << magneticField[0] << "," << magneticField[1]
                              << "," << magneticField[2] << std::endl;

                // Temperature.
                temperature = sensor[36];
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "temperature: " << (int)temperature << std::endl;
                ...
                if (proxData[0] < 0) { proxData[0] = 0; }
                if (proxData[1] < 0) { proxData[1] = 0; }
                if (proxData[2] < 0) { proxData[2] = 0; }
                if (proxData[3] < 0) { proxData[3] = 0; }
                if (proxData[4] < 0) { proxData[4] = 0; }
                if (proxData[5] < 0) { proxData[5] = 0; }
                if (proxData[6] < 0) { proxData[6] = 0; }
                if (proxData[7] < 0) { proxData[7] = 0; }
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "prox: " << proxData[0] << "," << proxData[1] << ","
                              << proxData[2] << "," << proxData[3] << "," << proxData[4] << "," << proxData[5] << ","
                              << proxData[6] << "," << proxData[7] << std::endl;
                ...
                if (lightAvg < 0) { lightAvg = 0; }
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "lightAvg: " << lightAvg << std::endl;

                // ToF
                distanceMm = (uint16_t)((uint8_t)sensor[70] << 8) | ((uint8_t)sensor[69]);
                if (distanceMm > 2000) { distanceMm = 2000; }
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "distanceMm: " << distanceMm << "(" << (int)sensor[69]
                              << "," << (int)sensor[70] << ")" << std::endl;
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "mic: " << micVolume[0] << "," << micVolume[1] << ","
                              << micVolume[2] << "," << micVolume[3] << std::endl;
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "steps: " << motorSteps[0] << "," << motorSteps[1]
                              << std::endl;

                // Battery
                batteryRaw = (uint8_t)sensor[83] + (uint8_t)sensor[84] * 256;
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "batteryRaw: " << batteryRaw << std::endl;

                // Micro sd state.
                microSdState = sensor[85];
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "microSdState: " << (int)microSdState << std::endl;
                ...
                irData    = sensor[88];
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "tv remote: " << (int)irCheck << "," << (int)irAddress
                              << "," << (int)irData << std::endl;

                // Selector.
                selector = sensor[89];
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "selector: " << (int)selector << std::endl;
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "groundProx: " << groundProx[0] << "," << groundProx[1]
                              << "," << groundProx[2] << std::endl;
                ...
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "groundAmbient: " << groundAmbient[0] << ","
                              << groundAmbient[1] << "," << groundAmbient[2] << std::endl;

                // Button state.
                buttonState = sensor[102];
                if (DEBUG_UPDATE_SENSORS_DATA)
                    std::cout << "[" << epuckname << "] " << "buttonState: " << (int)buttonState << std::endl;

                break;

            case 0x03: printf("empty packet\r\n"); break;

            default: printf("unexpected packet\r\n"); break;
            }
    ...
    void RGB565toRGB888(int width, int height, unsigned char *src, unsigned char *dst)
    {
        ...
        for (line = 0; line < height; ++line)
        {
            for (column = 0; column < width; ++column)
            {
                ...
                dst[index_dst++] =
                    (unsigned char)((src[index_src] & 0x07) << 5) | (unsigned char)((src[index_src + 1] & 0xE0) >> 3);
    ...
    {
        // Publish the static transforms for the sensors once, as they don't change relative to one another.
        ...
        auto MakeTfMsg = [](const tf2::Transform &transform, const rclcpp::Time &time, auto child, auto parent) {
            ...
            tf_msg.header.frame_id = std::move(parent);
            tf_msg.child_frame_id  = std::move(child);
    ...
    void updateRosInfo()
    {
        ...
        for (i = 0; i < 8; i++)
        {
            if (proxData[i] > 0)
            {
                proxMsg[i].range = 0.5 / sqrt(proxData[i]); // Transform the analog value to a distance value in meters
                                                            // (given from field tests).
            } else
            {
                proxMsg[i].range = proxMsg[i].max_range;
            }
            if (proxMsg[i].range > proxMsg[i].max_range) { proxMsg[i].range = proxMsg[i].max_range; }
            if (proxMsg[i].range < proxMsg[i].min_range) { proxMsg[i].range = proxMsg[i].min_range; }
            proxMsg[i].header.stamp = this->get_clock()->now();
        ...
        // #############################################################################################################################################
        //  Laserscan topic
        currentTimeMap = this->get_clock()->now();
        parent.str("");
        parent << epuckname << "/base_laser";
        ...
        // laserMsg.time_increment = (currentTimeMap-lastTimeMap).toSec()/180; //0.003; //(1 / laser_frequency) /
        // (num_readings); laserMsg.scan_time = (currentTimeMap-lastTimeMap).toSec();
        //  The laser is placed in the center of the robot, but the proximity sensors are placed around the robot thus
        //  add "ROBOT_RADIUS" to get correct values.
        ...
        lastTimeMap = this->get_clock()->now();

        // We use the information from the 6 proximity sensors on the front side of the robot to get 19 laser scan
        // points. The interpolation used is the following: -90 degrees: P2 -80 degrees: 4/5*P2 + 1/5*P1 -70 degrees:
        // 3/5*P2 + 2/5*P1 -60 degrees: 2/5*P2 + 3/5*P1 -50 degrees: 1/5*P2 + 4/5*P1 -40 degrees: P1 -30 degrees: 2/3*P1
        // + 1/3*P0 -20 degrees: 1/3*P1 + 2/3*P0 -10 degrees: P0 0 degrees: 1/2*P0 + 1/2*P7 10 degrees: P7 20 degrees:
        // 1/3*P6 + 2/3*P7 30 degrees: 2/3*P6 + 1/3*P7 40 degrees: P6 50 degrees: 1/5*P5 + 4/5*P6 60 degrees: 2/5*P5 +
        // 3/5*P6 70 degrees: 3/5*P5 + 2/5*P6 80 degrees: 4/5*P5 + 1/5*P6 90 degrees: P5
        ...
        if (tempProx > 0)
        {
            laserMsg.ranges[0] =
                (0.5 / sqrt(tempProx))
                + ROBOT_RADIUS; // Transform the analog value to a distance value in meters (given from field tests).
        ...
        for (i = 0; i < 19; i++)
        {
            if (laserMsg.ranges[i] > laserMsg.range_max) { laserMsg.ranges[i] = laserMsg.range_max; }
            if (laserMsg.ranges[i] < laserMsg.range_min) { laserMsg.ranges[i] = laserMsg.range_min; }
        }
        ...
        // #############################################################################################################################################
        //  Odometry topic
        //  The encoders values coming from the e-puck are 2 bytes signed int thus we need to handle the overflows
        //  otherwise the odometry will be wrong after a while (about 4 meters).
        ...
        if (DEBUG_ODOMETRY)
            std::cout << "[" << epuckname << "] " << "left, right raw: " << motorSteps[0] << ", " << motorSteps[1]
                      << std::endl;
        if (DEBUG_ODOMETRY)
            std::cout << "[" << epuckname << "] " << "left, right raw corrected: " << motorPositionDataCorrect[0]
                      << ", " << motorPositionDataCorrect[1] << std::endl;
        ...
        if (DEBUG_ODOMETRY)
            std::cout << "[" << epuckname << "] " << "left, right steps diff: " << leftStepsDiff << ", "
                      << rightStepsDiff << std::endl;

        deltaTheta = (rightStepsDiff - leftStepsDiff) / WHEEL_SEPARATION; // Expressed in radiant.
        deltaSteps = (rightStepsDiff + leftStepsDiff) / 2;                // Expressed in meters.
        if (DEBUG_ODOMETRY)
            std::cout << "[" << epuckname << "] " << "delta theta, steps: " << deltaTheta << ", " << deltaSteps
                      << std::endl;
        ...
        if (DEBUG_ODOMETRY)
            std::cout << "[" << epuckname << "] " << "x, y, theta: " << xPos << ", " << yPos << ", " << theta
                      << std::endl;
        ...
        odomMsg.twist.twist.linear.x =
            deltaSteps
            / ((currentTime - lastTime).seconds()); // "deltaSteps" is the linear distance covered in meters from the
                                                    // last update (delta distance); the time from the last update is
                                                    // measured in seconds thus to get m/s we multiply them.
        odomMsg.twist.twist.angular.z =
            deltaTheta
            / ((currentTime - lastTime).seconds()); // "deltaTheta" is the angular distance covered in radiant from the
                                                    // last update (delta angle); the time from the last update is
                                                    // measured in seconds thus to get rad/s we multiply them.
        if (DEBUG_ODOMETRY)
            std::cout << "[" << epuckname << "] " << "time elapsed = " << (currentTime - lastTime).seconds()
                      << " seconds" << std::endl;
        ...
        odomLastPublished = odomMsg.header.stamp;
        // #############################################################################################################################################
        ...
        imuMsg.linear_acceleration.x =
            (accData[1]) / 800.0 * STANDARD_GRAVITY; // 1 g = about 800, then transforms in m/s^2.
        ...
        if (DEBUG_IMU)
            std::cout << "[" << epuckname << "] " << "accel raw: " << accData[0] << ", " << accData[1] << ", "
                      << accData[2] << std::endl;
        if (DEBUG_IMU)
            std::cout << "[" << epuckname << "] " << "accel (m/s2): " << imuMsg.linear_acceleration.x << ", "
                      << imuMsg.linear_acceleration.y << ", " << imuMsg.linear_acceleration.z << std::endl;
        ...
        if (DEBUG_IMU)
            std::cout << "[" << epuckname << "] " << "gyro raw: " << gyroRaw[0] << ", " << gyroRaw[1] << ", "
                      << gyroRaw[2] << std::endl;
        if (DEBUG_IMU)
            std::cout << "[" << epuckname << "] " << "gyro (rad/s): " << imuMsg.angular_velocity.x << ", "
                      << imuMsg.angular_velocity.y << ", " << imuMsg.angular_velocity.z << std::endl;
        ...
        double pitch =
            atan2(-imuMsg.linear_acceleration.x, sqrt(imuMsg.linear_acceleration.y * imuMsg.linear_acceleration.y
        ...
        magFieldMsg.header.stamp                 = this->get_clock()->now();
        ...
        if (DEBUG_MAG_FIELD)
            std::cout << "[" << epuckname << "] " << "mag field (Tesla): " << magFieldMsg.magnetic_field.x << ", "
                      << magFieldMsg.magnetic_field.y << ", " << magFieldMsg.magnetic_field.z << std::endl;
        ...
        magFieldVectorMsg.header.stamp    = this->get_clock()->now();
        ...
        double mod = sqrt(magFieldMsg.magnetic_field.x * magFieldMsg.magnetic_field.x
                          + magFieldMsg.magnetic_field.y * magFieldMsg.magnetic_field.y);
        // End point
        ...
        microphoneMsg.header.stamp    = this->get_clock()->now();
        ...
        if (distSensMsg.range > distSensMsg.max_range) { distSensMsg.range = distSensMsg.max_range; }
        if (distSensMsg.range < distSensMsg.min_range) { distSensMsg.range = distSensMsg.min_range; }
        distSensMsg.header.stamp = this->get_clock()->now();
        ...
        if (ground_sensors_enabled)
        {
            ...
            floorMsg.header.stamp    = this->get_clock()->now();
            ...
            ss.str(std::string());
            ss << "floor: [" << groundProx[0] << ", " << groundProx[1] << ", " << groundProx[2] << ", " << groundProx[3]
               << ", " << groundProx[4] << "]";
    ...
    void handlerVelocity(const geometry_msgs::msg::Twist::ConstSharedPtr &msg)
    {
    ...
    void handlerLED(const std_msgs::msg::UInt8MultiArray::ConstSharedPtr &msg)
    {
    ...
    void handlerRgbLeds(const std_msgs::msg::UInt8MultiArray::ConstSharedPtr &msg)
    {
        ...
        if (DEBUG_RGB_RECEIVED)
        {
            ...
            {
                std::cout << i << ": " << msg->data[i * 3] << ", " << msg->data[i * 3 + 1] << ", "
                          << msg->data[i * 3 + 2];
            }
        }
    }
    ...
    bool init()
    {
        ...
        this->get_parameter("epuck2_id", robotId);
        this->get_parameter_or<std::string>("epuck2_address", epuckAddress, "");
        this->get_parameter_or<uint16_t>("epuck2_port", epuckPort, 1000);
        this->get_parameter_or<std::string>("epuck2_name", epuckname, "epuck2");
        this->get_parameter("robot/xpos", init_xpos);
        this->get_parameter("robot/ypos", init_ypos);
        this->get_parameter("robot/theta", init_theta);
        this->get_parameter("robot/camera", camera_enabled);
        this->get_parameter("robot/floor", ground_sensors_enabled);

        // this->get_parameter_or("ros_rate", rosRate, 7);

        if (DEBUG_ROS_PARAMS)
        {
            std::cout << "[" << epuckname << "] " << "epuck id: " << robotId << std::endl;
            std::cout << "[" << epuckname << "] " << "epuck address: " << epuckAddress << std::endl;
            std::cout << "[" << epuckname << "] " << "epuck port: " << epuckPort << std::endl;
            std::cout << "[" << epuckname << "] " << "epuck name: " << epuckname << std::endl;
            std::cout << "[" << epuckname << "] " << "init pose: " << init_xpos << ", " << init_ypos << ", " << theta
                      << std::endl;
        ...
        if (camera_enabled)
        {
            imagePublisher = image_transport::create_publisher(this, "camera", {RMW_QOS_POLICY_HISTORY_KEEP_LAST, 1});
         ...
         * called handlerVelocity.  create_subscription() returns a Subscription pointer that you
         * must hold on to until you want to unsubscribe.
         *
         * The second parameter to the create_subscription() function is the size of the message
         * queue.  If messages are arriving faster than they are being processed, this
         * is the number of messages that will be buffered up before beginning to throw
         * away the oldest ones.
         */
        cmdVelSubscriber = this->create_subscription<geometry_msgs::msg::Twist>(
            "mobile_base/cmd_vel", 10,
            [&](const geometry_msgs::msg::Twist::ConstSharedPtr &msg) { handlerVelocity(msg); });
        cmdLedSubscriber = this->create_subscription<std_msgs::msg::UInt8MultiArray>(
            "mobile_base/cmd_led", 10,
            [&](const std_msgs::msg::UInt8MultiArray::ConstSharedPtr &msg) { handlerLED(msg); });
        cmdRgbLedsSubscriber = this->create_subscription<std_msgs::msg::UInt8MultiArray>(
            "mobile_base/rgb_leds", 10,
            [&](const std_msgs::msg::UInt8MultiArray::ConstSharedPtr &msg) { handlerRgbLeds(msg); });
        ...
        tf_timer = this->create_timer(0.5s, [&](const rclcpp::TimerInfo &info) {
            // Only publish the odom transform if the last one was published more than 0.45s ago.
            ...
            tf2::Quaternion q;
            q.setRPY(0, 0, theta);
            odomTrans.transform.rotation = tf2::toMsg(q);
            br->sendTransform(odomTrans);
        });
...
int main(int argc, char *argv[])
{
    ...
    using namespace std::chrono_literals;
    rclcpp::Rate loop_rate(0.1s);

    auto node = std::make_shared<EpuckDriver>();
    if (!node->init()) { exit(1); }
